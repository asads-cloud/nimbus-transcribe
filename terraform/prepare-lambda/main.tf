###############################################################################
# main.tf — Infrastructure for "Prepare" Lambda
#
# What this does:
# - Creates S3 buckets for ingest (source audio + chunks/manifests) and results
# - Builds a Lambda Layer with FFmpeg binaries
# - Provisions IAM role + policies for Lambda
# - Deploys the Prepare Lambda function (handler.py)
# - Connects S3 bucket events (prefix=audio/) to trigger the Lambda
###############################################################################
...
locals {
  ...
  # Resolve absolute file paths for zip artifacts (important for hashing)
...
resource "aws_iam_role" "openai_prepare_role" {
  name               = "openai-prepare-lambda-role"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
  tags               = local.tags
}

# Inline policy: logs + S3 access
data "aws_iam_policy_document" "openai_prepare_inline" {
  # CloudWatch logs
  statement {
    sid       = "Logs"
...
resource "aws_iam_policy" "openai_prepare_policy" {
  name   = "openai-prepare-lambda-policy"
  policy = data.aws_iam_policy_document.openai_prepare_inline.json
}

resource "aws_iam_role_policy_attachment" "attach_prepare" {
  role       = aws_iam_role.openai_prepare_role.name
  policy_arn = aws_iam_policy.openai_prepare_policy.arn
}

# ── Lambda Function (Prepare) ────────────────────────────────────────────────
resource "aws_lambda_function" "openai_prepare" {
  function_name    = "openai-prepare-chunking"
  role             = aws_iam_role.openai_prepare_role.arn
  handler          = "handler.lambda_handler"
  runtime          = "python3.11"
  architectures    = ["x86_64"] # Layer is amd64 static build
...
resource "aws_lambda_permission" "allow_s3" {
  ...
  function_name = aws_lambda_function.openai_prepare.function_name
  principal     = "s3.amazonaws.com"
  source_arn    = aws_s3_bucket.ingest.arn
}

# ── S3 -> Lambda Event Notifications ─────────────────────────────────────────
# One block per suffix to catch common audio formats under prefix=audio/
resource "aws_s3_bucket_notification" "ingest_events" {
  ...
  dynamic "lambda_function" {
    ...
    content {
      lambda_function_arn = aws_lambda_function.openai_prepare.arn
