###############################################################################
# collect-phase3.ps1 — Validate results + capture Batch/IAM metadata
#
# What this script does:
# 1) Resolves account and bucket names; builds an S3 prefix for a given job id
# 2) Verifies that at least one transcript object exists under that prefix
# 3) Downloads a representative JSON result locally for quick preview
# 4) Collects ARNs for Batch compute env / queue / job def + IAM roles
# 5) Writes artifacts/phase3.json (consumed by later steps)
#
# Prerequisites:
# - AWS CLI configured (credentials + default region or pass --region)
# - Results already uploaded under s3://<results-bucket>/chunks/<jobId>/...
###############################################################################

# ── Inputs you already have (adjust the '<< set me >>' line only) ────────────
$Region        = "eu-west-1"
...
# If you used the tutorial job id, set it here (affects what we scan/download)
# Example from your last success: task-0002
...
# ── 1) Verify there is at least one transcript under the prefix ─────────────
...
# ── 2) Preview the first result locally (rename sensibly if key == prefix) ──
...
# ── Helpers compatible with Windows PowerShell 5.1 ──────────────────────────
...
# Defensive extraction (no ?. to keep PS 5.1-friendly)
$computeEnvArn = $null
...
# ── Persist to artifacts/phase3.json ─────────────────────────────────────────
